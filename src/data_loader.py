        ...
        with open(path, "r") as f:
            ...
            for item_data in data:
                try:
                    ...
                    # Validate that all required seeds exist
                    required_seed_reqs = payoff.required_seeds.seeds
                    for seed_req in required_seed_reqs:
                        if seed_req.id not in all_seeds:
                            raise ValueError(
                                f"Payoff '{item_id}' references an unknown seed '{seed_req.id}'"
                            )
